...
const endpoints = {
    ...
    createShareLink: (id: string) =>
        `https://api.music-flo.com/common/v1/sharing/url?shareType=VIEW_CONTENT&type=PLAYLIST&id=${id}`,
}
...
export const generateURL = async (playlist: Playlist): Promise<string> => {
    const generatePlaylistRes = await axios(endpoints.createPlaylist(), {
        method: "POST",
        headers: {
    ...
    const createShareLinkRes = await axios(
        endpoints.createShareLink(generatePlaylistRes.data.data.id)
    )

    return createShareLinkRes.data.data.id
}
